...
from ramsey_elimination.formula_utils import _fresh_vector, collect_atoms, fresh_bool_vector
from ramsey_elimination.simplifications import make_euf_input_format
...
def eliminate_ramsey_euf(qFormula: ExtendedFNode, N = 4) -> ExtendedFNode:
    ...
    prop_skeleton = qFormula.arg(0).substitute({atom: skeleton_vars[i] for i, atom in enumerate(eqs)})
    theory_con = And([Implies(skeleton_vars[i], eqs[i]) for i in range(len(eqs))])

    qargs: Tuple[Tuple[ExtendedFNode, ...], Tuple[ExtendedFNode, ...]] = qFormula.quantifier_vars() #type: ignore
    n = len(qargs[0])
    T = qargs[0][0].get_type()

    vecs = [_fresh_vector("x_{}_%s", n, T) for _ in range(N)]
    ...
    clique = And([
        theory_con.substitute(dict(zip(formals, xs_p + ys_p)))
        for xs_p, ys_p in combinations(vecs, 2)
    ])

    return Exists(skeleton_vars + [x for vec in vecs for x in vec], And(uneq, clique, prop_skeleton))
